...
Use `.ai/plugin-team.yaml`, `.ai/plugin-registry.yaml`, `.ai/specialist-router.yaml` and `.ai/resilience-policy.yaml` to coordinate the project plugin team from start to finish. Core project specialists include Context7, Exa, Neon, GitHub, Vercel, Figma and Canva. They share the workboard and must publish cross-lane discoveries through the Controller.
...
Do not invoke every specialist on every task. Keep the whole team available, but activate only lanes that materially improve the current Task Execution Envelope. The Controller owns the single GitHub Actions runner queue; while that runner is busy or unavailable, non-runner lanes continue in parallel. GitHub or plugin unavailability must never become a global stop unless no safe useful unblocked lane remains.
